...
The selection SHALL be carried as `?local=`, `?servicio=` and `?barbero=`, so that back, forward, reload and a shared link all restore the same step.
...
Changing an upstream selection SHALL discard every downstream one. Carrying a service from a branch the client just changed produces an inconsistent triple that would have to be caught later anyway.

The links shared on WhatsApp outlive the catalogue they were built from. A stale link is the normal case, not the exception, and 404ing the whole page for a barber who was deactivated last week discards a branch and a service choice that are both still correct.
...
- **THEN** the service and barber selections are discarded rather than carried forward
...
`durationMinutes` is included although this change does not render it: B3 sizes slots by it and would otherwise re-issue the query.
