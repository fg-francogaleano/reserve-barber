...
- [ ] 11.1 Deploy the cron Worker with no reminder cron expression. The job is unreachable code; the deploy proves the bundle. — `npx wrangler deploy -c wrangler.cron.jsonc`
- [ ] 11.2 Set `APP_ORIGIN` and `EMAIL_FROM` on the cron Worker. **Leave `RESEND_API_KEY` unset**, per T76. `APP_ORIGIN` is already committed in `vars`; `EMAIL_FROM` stays absent until a domain is verified.
...
- [ ] 11.4 Confirm the schedule is registered on the deployed Worker and that the first invocation emits a summary under `email.bookingReminder`.
