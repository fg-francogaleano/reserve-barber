...
- [x] 1.2b **ANSWERED by measurement, 2026-08-22.** Franco swapped the stored credential to his **production** one and `/v1/payment_methods` was queried again with it, decrypted from the database — the credential the flow actually charges with. **Identical to the test account**: 16 active methods, none inactive, in the same four bands (prepaid 1, debit + Visa/Mastercard/Amex 3, Diners/Naranja/Argencard/Cabal 15, cash tickets 50), and the same 1,000,000 ARS ceiling on the tickets. `MIN_DEPOSIT_AMOUNT = 15.00` holds against the account that will bill. The hedge existed because the first measurement used a token handed to the gate rather than the stored one; that gap is now closed by measurement rather than by assumption.
