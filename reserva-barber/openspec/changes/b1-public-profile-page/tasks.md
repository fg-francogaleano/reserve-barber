...
- [ ] 10.4 **BLOCKED — needs a deploy, and only a deploy.** Paste the shareable link into a real WhatsApp message and confirm the preview renders with the business name and cover image. `APP_ORIGIN` is configured now (10.7) and the tags were verified present locally — `canonical`, `og:title`, `og:description`, `og:url`, `og:locale` and `og:image` all emitted. What remains is purely that WhatsApp has to fetch a publicly reachable origin, and localhost is not one. Run this immediately after the first deploy
