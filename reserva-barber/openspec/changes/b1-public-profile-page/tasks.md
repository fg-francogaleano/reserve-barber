...
- [ ] 10.4 **UNBLOCKED — one manual step left, for the owner.** Deployed 2026-08-15 (version `51c956ba`). Verified against production: every path behaves as specified (`200` / `308` to canonical / `404` / `307`), and all seven link-preview tags are emitted with the production origin — `canonical`, `og:title`, `og:description`, `og:url`, `og:locale`, `og:image`, `og:type`. What remains cannot be automated from here: paste `https://reserva-barber.franco-galeano.workers.dev/b/barberia-don-juan-centro` into a real WhatsApp message and confirm the card renders with the business name and cover image. Sending a message on the owner's behalf is theirs to do
