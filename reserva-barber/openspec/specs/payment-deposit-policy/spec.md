# payment-deposit-policy Specification

## Purpose
The owner decides how much of a service's price a client pays up front to confirm a booking — a fixed amount or a whole percentage — and sees what that charges against their real service prices before storing it. Carries the single authoritative deposit calculation, with its rounding, its cap at the service price and its minimum floor, which the booking and payment stories consume rather than reimplement. Also the one place that answers whether the business can accept bookings at all. Created by archiving change pc3-deposit-policy.
## Requirements
