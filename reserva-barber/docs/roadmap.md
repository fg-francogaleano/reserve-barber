...
- [ ] **B2** — As a client, I want to choose a location, then a service, then a barber of that location who performs it, so that my booking matches how the business operates. — *depends on: B1, M4*
  - **Inherited from B1, decided and not assumed:** the bookability gate is B2's. B1 renders "Reservar" for every published profile regardless of whether the owner has a location, a barber, an assigned service or a deposit policy, so B2 owns what a client meets when the catalogue is empty.
  - **The public route must keep away from `PaymentConfig`.** B1's composition root deliberately constructs no Supabase client and no cipher, and hands over no `PaymentConfig` repository — that row holds the encrypted Mercado Pago access token, and PC3 already established that a surface with no need for a cipher must not build one. If B2 needs to know whether a service is bookable, derive it from `BarberService` and the catalogue, not from the payment row.
  - The public shell (`app/b/layout.tsx`) and the guard exception already exist; B2 hangs `/b/{slug}/reservar` under them without revisiting either.
