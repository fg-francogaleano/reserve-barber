...
- `confirmationEmailSentAt`: When the email provider accepted the confirmation message (**`Timestamptz(3)`**, optional — zone-aware, never the zone-less default)
- `reminderEmailSentAt`: When a reminder was **claimed** for this booking (**`Timestamptz(3)`**, optional — zone-aware). **Opposite in kind to the column above it**, see below
- `createdAt` / `updatedAt`: Timestamps
...
- **`cancelledAt` and `cancelledBy` gain their first writers in C2, and every writer of `CANCELLED` must set them.** The columns and the `CancelledBy` enum were created whole by B3 and stayed null for three stories: B6's receipt rejection — the only producer of `CANCELLED` in the product until C2 — wrote the status alone. That had three consequences nobody saw until a second canceller was designed. **The dashboard's cancellations counter could not count**, because it bounds on `cancelledAt` and no row had one. **The client's page could not attribute the decision**, because attribution keys on `cancelledBy`. And **once a client can cancel their own booking the two are indistinguishable**, with no later read able to recover which. `OWNER` covers both owner cancellation and receipt rejection — the owner is the actor in both — leaving `CLIENT` for C1. A cancellation **clears `holdExpiresAt`**, deliberately unlike an expiry, which preserves it as the evidence of why the row ended. Rows cancelled before C2 keep their nulls: **no backfill**, because inventing a canceller for historical rows would be a guess presented as data, where a null honestly records that nobody was tracking it
- **`CLIENT` gains its writer in C1, and the enum is now fully exercised.** The value existed from B3 and nothing had ever stored it. C1's token-scoped cancellation is its first and only writer, and the distinction the column was created for finally pays for itself in two places: the client's page renders a receipt of their own decision instead of the unattributed form, and the dashboard's recent-bookings list names who ended each cancelled booking — the only channel an owner has, because C1 deliberately sends them no message (`tech-debt.md` T77). The client's write is **narrower than the owner's**: it refuses a booking whose appointment has started, and refuses `PENDING_APPROVAL` outright, because a receipt awaiting review is money already transferred whose only record would vanish from the owner's queue. **Still no migration** — C1 stores a value the schema has admitted since B3
- **`confirmationEmailSentAt` records that the client was told, and it is NOT an idempotency key.** It is written after the email provider accepts the confirmation message, **outside** the confirming transaction and by a write that disturbs nothing about what the booking is — no status, no hold, no snapshot, no token. ( moves with it, as it does on every write through Prisma; the N1 gate caught an earlier version of this sentence claiming "no other column", which was never true.) Nothing reads it before sending: at-most-once delivery is already a property of the confirming transition itself, which is a conditional update guarded on the status it expects, so exactly one caller per booking ever observes the confirming outcome. A second mechanism claiming the same guarantee would be a second thing to get wrong — and a read-then-send would reintroduce precisely the race the guarded update exists to avoid. What the column buys is a question the product otherwise cannot ask: **a null on a `CONFIRMED` booking means that client was never told their appointment is real**, and that set is a `WHERE` clause rather than a log line nobody reads. Its own write failing changes nothing and is logged; a booking must never become unconfirmed because a bookkeeping write failed. It carries no index — nothing queries it yet, and the story that gives that set a surface is the story that should measure the predicate
- **`reminderEmailSentAt` IS the idempotency key, and it is the exact inverse of the column above. Read both bullets before generalising from either.** N2's trigger is **time passing**: nothing in the row changes to mark a booking as due, so there is no guarded transition to inherit at-most-once from, and the guarantee has to be constructed. The claim **is** the read — one conditional update setting this column `WHERE "reminderEmailSentAt" IS NULL AND status = 'CONFIRMED'`, returning what it matched — so a second run, two overlapping invocations and a hand-fired trigger each match zero rows. It is written **before** the send, which is also the reverse of N1's ordering: recording after acceptance leaves a window (a dying Worker, a provider that accepts and then times out) in which the row is unclaimed and the next invocation sends again, once per invocation, for as long as the appointment stays due — so claiming first bounds the failure to one client losing one reminder, while recording first bounds nothing. **Nothing un-claims a row after a failed send**, because a claimed row that could not be delivered may already have been delivered. Its presence therefore means "a reminder was **claimed**", deliberately not "a reminder was delivered"; a failed send is a log line and a `WHERE` clause. Unlike N1's column it **is** indexed, by the partial `Booking_reminder_due` — because a scheduled job queries it hourly across every owner, which is exactly the surface N1 recorded that its own column lacked
